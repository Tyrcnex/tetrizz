import { version } from "../../../utils/index.mjs";
import { basic } from "../../../utils/api/basic.mjs";
...
const extractPacker = (text)=>{
    const trace = (text, start, openStr = "{", closeStr = "}")=>{
        let i = start;
        let brace = 1;
    ...
    let libraries = text.slice("(() => {\n".length, text.indexOf('(function(){"use strict";'));
    const bufferStart = libraries.indexOf("e=function(){");
    const bufferEnd = trace(libraries, bufferStart + "e=function(){".length) + "()".length;
    libraries = libraries.slice(0, bufferStart) + "e=Buffer" + libraries.slice(bufferEnd);
    const pixiStart = libraries.search(/[A-Za-z]{1}=function\([A-Za-z]{1}\)\{"use strict";var [A-Za-z]{1}=setTimeout;/);
    let pixiEnd = trace(libraries, trace(libraries, pixiStart + "e=(function(t){".length) + '({}),s=function(t,e,r,n,i,o,s,a){"use strict";'.length);
    pixiEnd = libraries.indexOf(";", pixiEnd);
    libraries = libraries.slice(0, pixiStart - 1) + libraries.slice(pixiEnd) + "/*LIBRARY_CUTOFF*/";
    libraries = libraries.replace("o.filters", "{}");
    const assignIndex = libraries.search(/Object\.assign\(\{\},[A-Za-z]\);/);
    libraries = libraries.slice(0, assignIndex) + libraries.slice(assignIndex + "Object.assign({},s);".length);
    const mainStart = text.search(/class\s+[A-Za-z]{2}\s+extends\s+[A-Za-z]\{static\s+OptionsList/);
    const parentName = text.slice(mainStart + "class ge extends ".length, mainStart + "class ge extends ".length + 1);
    const parentStart = text.search(new RegExp(`class ${parentName}\\{constructor\\((.)\\)\\{this\\.self=\\1\\}`));
    const parentEnd = trace(text, parentStart + "class k{".length);
    const parentBody = text.slice(parentStart, parentEnd);
    const zenithStaticStart = text.search(new RegExp(`class [A-Za-z]{2} extends ${parentName}\\{\\s*static FloorDistance`));
    const zenithStaticEnd = text.indexOf("static GetSpeedCap(", zenithStaticStart);
    const zenithStaticBody = text.slice(zenithStaticStart, zenithStaticEnd) + "}";
    const bagStart = text.search(new RegExp(`class [A-Za-z]{1} extends ${parentName}\\{\\s*static BagList`));
    ...
    const counterStart = text.search(new RegExp(`class [A-Za-z]{1} extends ${parentName}\\{\\s*static DisplayCounters`));
    ...
    counterBody = counterBody.slice(0, counterBody.indexOf(counterEndStr) + counterEndStr.length) + "}";
    const engineStart = text.search(new RegExp(`class [A-Za-z]{1} extends ${parentName}\\{\\s*static init\\(\\)\\{this\\.ROTATION_LEFT=1`));
    const engineName = text.slice(engineStart + "class ".length, engineStart + "class ".length + 1);
    const engineEnd = trace(text, engineStart + "class H extends k{static init(){".length);
    const engineBody = text.slice(engineStart, engineEnd) + "}" + engineName + ".init();";
    const constantsStart = text.indexOf("={minotypes:") - 7;
    const constantsEnd = trace(text, constantsStart + "const w={minotypes:".length);
    ...
    const versionVar = "const " + text.slice(versionStart + versionString.length, versionStart + versionString.length + 1) + "=11;";
    const version2String = 'gameModes:{"40l":{version:';
    const version2Start = text.indexOf(version2String);
    const version2Var = "const " + text.slice(version2Start + version2String.length, version2Start + version2String.length + 1) + "=19;";
    const spinRulesStart = text.indexOf(".SpinRules=[") - 2;
    const spinRulesName = text.slice(spinRulesStart, spinRulesStart + 2);
    const spinRulesBody = "class " + spinRulesName + "{}" + text.slice(spinRulesStart, text.indexOf("];", spinRulesStart) + 2);
    ...
    return `(() => { ` + libraries + parentBody + versionVar + version2Var + zenithStaticBody + bagBody + counterBody + cacheBody + constantsBody + spinRulesBody + engineBody + text.slice(0, obfuscationEnd) + `return ${className}})();`;
};
export const tetoPack = (userAgent, options = {
    global: false
})=>{
    ...
    return new Promise(async (resolve)=>{
        ...
        let tetrioOverride;
        try {
            tetrioOverride = await fs.readFile(path.join(triangleDir, fileName), "utf-8");
        } catch  {
            log("TETR.IO update found. Extracting new packer...", {
                level: "warning"
            });
            if (!fsSync.existsSync(triangleDir)) {
                await fs.mkdir(triangleDir, {
                    recursive: true
                });
            }
            ...
            log(`tetrio.js @${serverVersion.signature.version}-${serverVersion.signature.client.build.id} patched`);
        }
        const tetrio = (0, eval)(tetrioOverride);
        Amadeus = {
            encode: (msg, data)=>{
                const encoded = tetrio.Encode(msg, data).toJSON().data;
                return Buffer.from([
                    ...encoded
                ]);
            },
            decode: (data)=>{
                ...
                if (decoded.command === "packets") {
                    decoded.data.packets = decoded.data.packets.map((packet)=>Buffer.from([
                            ...packet
                        ]));
                }
                return decoded;
...
//# sourceMappingURL=teto-pack.js.map