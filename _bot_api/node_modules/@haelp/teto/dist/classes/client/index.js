...
const _ribbon = require("../ribbon");
const _room = require("../room");
const _social = require("../social");
...
class Client {
   ...
   */ on;
    ...
    /** @hideconstructor */ constructor(token, sessionID, ribbon, me, userAgent = _utils.CONSTANTS.userAgent, handling){
        this.token = token;
        this.ribbon = ribbon;
        ...
        this.utils = new _utils1.ClientUtils(this);
        this.api = new _utils.API({
            token: this.token,
            userAgent
        });
        this.rooms = {
   ...
   */ static async connect(options) {
        ...
        if ("token" in options) {
            ...
            self = {
                token: options.token,
                id: (0, _utils.parseToken)(options.token)
            };
        ...
        const ribbon = await _ribbon.Ribbon.create({
            token,
            handling,
            userAgent: options.userAgent || _utils.CONSTANTS.userAgent,
            ...options.ribbon ?? {}
        });
        ...
        const client = new Client(token, sessionID, ribbon, await me, options.userAgent || _utils.CONSTANTS.userAgent, handling);
        client.social = await _social.Social.create(client, options.social || {}, data.social);
        return client;
    }
    /**
   ...
   */ wrap(event, data, listen, error = [
    ...
    /** @hidden */ init() {
        this.on("room.join", async ()=>{
            ...
            this.room = new _room.Room(this, data);
   ...
   */ async reconnect() {
        ...
        delete this.room;
        this.social = await _social.Social.create(this, this.social.config, data.social);
    }
