...
Object.defineProperty(exports, "LegacyGarbageQueue", {
    enumerable: true,
    get: function() {
        return LegacyGarbageQueue;
    }
});
...
class LegacyGarbageQueue {
    ...
    sent = 0;
    constructor(options){
        this.options = (0, _utils.deepCopy)(options);
        if (!this.options.cap.absolute) this.options.cap.absolute = Number.MAX_SAFE_INTEGER;
        this.queue = [];
        this.rng = new _utils.RNG(this.options.seed);
    }
    snapshot() {
        return {
            seed: this.rng.seed,
            ...
            sent: this.sent,
            queue: (0, _utils.deepCopy)(this.queue),
            hasChangedColumn: false,
            lastReceivedCount: 0
        };
    }
    fromSnapshot(snapshot) {
        this.queue = (0, _utils.deepCopy)(snapshot.queue);
        this.lastTankTime = snapshot.lastTankTime;
        this.lastColumn = snapshot.lastColumn;
        this.rng = new _utils.RNG(snapshot.seed);
   ...
   */ predict() {
        ...
        const result = this.#__internal_tank((0, _utils.deepCopy)(this.queue), ()=>lastColumn, rngex, reroll, -Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER, false);
        return result.res;
    }
    get nextColumn() {
        const rng = this.rng.clone();
    ...
    #__internal_tank(queue, lastColumn, rngex, reroll, frame, cap, hard) {
        ...
        while(total < cap && queue.length > 0){
            const item = (0, _utils.deepCopy)(queue[0]);
