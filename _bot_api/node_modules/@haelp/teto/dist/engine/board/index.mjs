...
export class Board {
    ...
    clearBombs(placedBlocks) {
        ...
        const bombColumns = lowestBlocks.filter(([x, y])=>this.state[y - 1][x] === Mino.BOMB).map(([x, _])=>x);
        if (bombColumns.length === 0) return {
            lines: 0,
            garbageCleared: 0
        };
        const lines = [];
        while(lowestY > 0 && bombColumns.some((col)=>this.state[lowestY - 1][col] === Mino.BOMB)){
            lines.push(--lowestY);
        }
    ...
    insertGarbage({ amount, size, column, bombs }) {
        this.state.splice(0, 0, ...Array.from({
            length: amount
                ...
                length: this.width
            }, (_, idx)=>idx >= column && idx < column + size ? bombs ? Mino.BOMB : null : Mino.GARBAGE)));
        this.state.splice(this.fullHeight - amount - 1, amount);
    }
}
export * from "./connected.mjs";
