...
export class ConnectedBoard {
    ...
    clearBombs(placedBlocks) {
        ...
        const bombColumns = lowestBlocks.filter(([x, y])=>this.state[y - 1][x]?.mino === Mino.BOMB).map(([x, _])=>x);
        if (bombColumns.length === 0) return {
            lines: 0,
            garbageCleared: 0
        };
        const lines = [];
        while(lowestY > 0 && bombColumns.some((col)=>this.state[lowestY - 1][col]?.mino === Mino.BOMB)){
            lines.push(--lowestY);
        }
