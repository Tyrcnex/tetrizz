...
const _events = require("../utils/events");
const _board = _export_star(require("./board"), exports);
const _constants = require("./constants");
const _garbage = _export_star(require("./garbage"), exports);
const _multiplayer = _export_star(require("./multiplayer"), exports);
const _queue = _export_star(require("./queue"), exports);
const _utils = _export_star(require("./utils"), exports);
const _damageCalc = require("./utils/damageCalc");
const _kicks = require("./utils/kicks");
const _data = require("./utils/kicks/data");
const _tetromino = require("./utils/tetromino");
...
class Engine {
    ...
    resCache;
    constructor(options){
        this.initializer = (0, _utils.deepCopy)(options, [
            {
                type: Date,
                copy: (d)=>new Date(d)
            }
    ...
    init() {
        const options = (0, _utils.deepCopy)(this.initializer, [
            {
                type: Date,
                copy: (d)=>new Date(d)
            }
        ]);
        this.queue = new _queue.Queue(options.queue);
        this.queue.onRepopulate(this.#onQueueRepopulate.bind(this));
        this._kickTable = options.kickTable;
        this.board = new _board.Board(options.board);
        this.connectedBoard = new _board.ConnectedBoard(options.board);
        this.garbageQueue = new ((options.misc.date ?? new Date()) > new Date("2025-05-06T15:00:00-04:00") ? _garbage.GarbageQueue : _garbage.LegacyGarbageQueue)(options.garbage);
        this.igeHandler = new _multiplayer.IGEHandler(options.multiplayer?.opponents || []);
        ...
        this.dynamic = {
            gravity: new _utils.IncreaseTracker(options.gravity.value, options.gravity.increase, options.gravity.marginTime),
            garbageMultiplier: new _utils.IncreaseTracker(options.garbage.multiplier.value, options.garbage.multiplier.increase, options.garbage.multiplier.marginTime),
            garbageCap: new _utils.IncreaseTracker(options.garbage.cap.value, options.garbage.cap.increase, options.garbage.cap.marginTime)
        };
        this.glock = 0;
    ...
    flushRes() {
        const res = this.resCache ? (0, _utils.deepCopy)(this.resCache) : null;
        this.resCache = {
            pieces: 0,
            garbage: {
                sent: [],
    ...
    fromSnapshot(snapshot) {
        ...
        this.board.state = (0, _utils.deepCopy)(snapshot.board);
        this.connectedBoard.state = (0, _utils.deepCopy)(snapshot.connectedBoard);
        this.falling = new _tetromino.Tetromino({
        ...
        for (const key of Object.keys(snapshot.falling)){
            // @ts-expect-error
            this.falling[key] = (0, _utils.deepCopy)(snapshot.falling[key]);
        }
        ...
        this.lastSpin = (0, _utils.deepCopy)(snapshot.lastSpin);
        ...
        for(let i = 0; i < this.frame; i++)for (const key of Object.keys(this.dynamic))this.dynamic[key].tick();
        this.input = (0, _utils.deepCopy)(snapshot.input);
        if (this.multiplayer && snapshot.targets) this.multiplayer.targets = [
            ...snapshot.targets
        ];
        this.stats = (0, _utils.deepCopy)(snapshot.stats);
        this.glock = snapshot.glock;
        this.state = snapshot.state;
        this.currentSpike = snapshot.currentSpike;
        this.igeHandler.fromSnapshot(snapshot.ige);
        this.resCache = (0, _utils.deepCopy)(snapshot.resCache);
    }
    get kickTable() {
        return _data.kicks[this._kickTable];
    }
    get kickTableName() {
    ...
    #hasHitWall() {
        return !!(this.state & _constants.constants.flags.STATE_WALL);
    }
    // @ts-expect-error unused
    #hasRotated() {
        return !!(this.state & (_constants.constants.flags.ROTATION_LEFT | _constants.constants.flags.ROTATION_RIGHT));
    }
    // @ts-expect-error unused
    #hasRotated180() {
        return !!(this.state & _constants.constants.flags.ROTATION_180);
    }
    // @ts-expect-error unused
    #isSpin() {
        return !!(this.state & _constants.constants.flags.ROTATION_SPIN);
    }
    // @ts-expect-error unused
    #isSpinMini() {
        return !(~this.state & (_constants.constants.flags.ROTATION_SPIN | _constants.constants.flags.ROTATION_MINI));
    }
    // @ts-expect-error unused
    #isSpinAll() {
        return !!(this.state & _constants.constants.flags.ROTATION_SPIN_ALL);
    }
    #isSleep() {
        return !!(this.state & _constants.constants.flags.STATE_SLEEP);
    }
    // @ts-expect-error unused
    #isFloored() {
        return !!(this.state & _constants.constants.flags.STATE_FLOOR);
    }
    // @ts-expect-error unused
    #isVisible() {
        return !(this.state & _constants.constants.flags.STATE_NODRAW);
    }
    // @ts-expect-error unused
    #isSoftDropped() {
        return !!(this.state & _constants.constants.flags.ACTION_SOFTDROP);
    }
    #isForcedToLock() {
        return !!(this.state & _constants.constants.flags.ACTION_FORCELOCK);
    }
    #is20G() {
    ...
    #__internal_fall(value) {
        ...
        if (y1 % 1 === 0) y1 -= 1e-6;
        if (y2 % 1 === 0) y2 += 2e-6;
        if (!(0, _kicks.legal)(this.falling.absoluteAt({
            y: y1
        }), this.board.state) || !(0, _kicks.legal)(this.falling.absoluteAt({
            y: y2
        ...
        this.state &= ~_constants.constants.flags.STATE_FLOOR;
    ...
    #fall(subframe = 1 - this.subframe) {
        ...
        if (this.#shouldLock() && !(0, _kicks.legal)(this.falling.absoluteAt({
            y: this.falling.location[1] - 1
        }), this.board.state)) {
            fall = 20;
            this.state |= _constants.constants.flags.ACTION_FORCELOCK;
        }
        ...
        for(let dropFactor = fall; dropFactor > 0; dropFactor -= Math.min(1, dropFactor)){
            ...
            if (Math.floor(y) !== Math.floor(this.falling.location[1])) {
                this.state &= ~_constants.constants.flags.ROTATION_ALL;
            }
        }
    }
    ...
    #__internal_rotate(newX, newY, newRotation, rotationDirection, kick, _isIRS = false) {
        ...
        if (rotationDirection >= 2) {
            ...
            this.state |= _constants.constants.flags.ROTATION_180;
        }
        ...
        this.state |= rotationDirection === 1 ? _constants.constants.flags.ROTATION_RIGHT : _constants.constants.flags.ROTATION_LEFT;
        this.state &= ~(_constants.constants.flags.ROTATION_SPIN | _constants.constants.flags.ROTATION_MINI | _constants.constants.flags.ROTATION_SPIN_ALL);
        ...
        if (spin) {
            this.state |= _constants.constants.flags.ROTATION_SPIN;
            if (spin === "mini") {
                this.state |= _constants.constants.flags.ROTATION_MINI;
            }
        }
    ...
    #kick(to) {
        const kick = (0, _kicks.performKick)(this.kickTableName, this.falling.symbol, this.falling.location, [
            this.falling.aox,
            this.falling.aoy
    ...
    #rotate(amount, isIRS = false) {
        ...
        this.state |= _constants.constants.flags.ACTION_MOVE;
        this.state |= _constants.constants.flags.ACTION_ROTATE;
    ...
    initiatePiece(piece, ignoreBlockout = false, isHold = false) {
        ...
        if (this.handling.ihs === "hold" && this.input.keys.hold && !isHold) {
            this.state |= _constants.constants.flags.ACTION_IHS;
        }
        this.#__internal_dcd();
        this.state &= ~(_constants.constants.flags.ROTATION_ALL | _constants.constants.flags.STATE_ALL | _constants.constants.flags.ACTION_FORCELOCK | _constants.constants.flags.ACTION_SOFTDROP | _constants.constants.flags.ACTION_MOVE | _constants.constants.flags.ACTION_ROTATE);
        ...
        if (!ignoreBlockout && this.#considerBlockout(isHold)) {
            ...
            if (this.state & _constants.constants.flags.ACTION_IHS) {
                this.state ^= _constants.constants.flags.ACTION_IHS;
    ...
    #considerBlockout(isSilent = false) {
        if ((0, _kicks.legal)(this.falling.absoluteBlocks, this.board.state)) {
            // TODO: clutch count
            // if (!isSilent) {
    ...
    hold(_ihs = false, ignoreBlockout = false) {
        if (this.#isSleep()) {
            if (this.handling.ihs === "tap") this.state |= _constants.constants.flags.ACTION_IHS;
            return false;
        }
    ...
    #spinbonuses(piece) {
        const p = _tetromino.tetrominoes[piece];
        const rules = _data.spinbonusRules[this.gameOptions.spinBonuses];
        // @ts-expect-error
        return p?.spinbonus_override ? p.spinbonus_override?.mini : !!rules?.types_mini?.includes(piece);
    }
    #detectSpinFromCorners(finOrTst) {
        if ((0, _kicks.legal)(this.falling.blocks.map((block)=>[
                block[0] + this.falling.location[0],
                -block[1] + this.falling.y - 1
            ]), this.board.state)) return "none";
        let corners = 0;
        let frontCorners = 0;
        for(let i = 0; i < 4; i++){
            const table = _data.cornerTable[this.falling.symbol]?.[this.falling.rotation];
            if (!table) break;
    ...
    #lock(hard) {
        ...
        this.connectedBoard.add(...this.connect(placed.map(([_, x, y])=>[
                x,
                -y
            ])).map(([x, y, s])=>[
                {
        ...
        this.connectedBoard.clearBombsAndLines(placed.map((b)=>[
                b[1],
                b[2]
            ]));
        const { lines, garbageCleared } = this.board.clearBombsAndLines(placed.map((b)=>[
                b[1],
                b[2]
            ]));
        ...
        const gSpecialBonus = this.garbageQueue.options.specialBonus && garbageCleared > 0 && (this.lastSpin && this.lastSpin.type !== "none" || lines >= 4) ? 1 : 0;
        const garbage = (0, _damageCalc.garbageCalcV2)({
        ...
        this.lastSpin = null;
        try {
            if (!(0, _kicks.legal)(this.falling.absoluteBlocks, this.board.state)) res.topout = true;
        } catch  {
            res.topout = true;
        }
        if (res.garbage.length > 0) {
            if (this.multiplayer) this.multiplayer.targets.forEach((target)=>res.garbage.forEach((g)=>this.igeHandler.send({
                        amount: g,
                        playerID: target
                    })));
        }
        ...
        this.events.emit("falling.lock", (0, _utils.deepCopy)(res));
        return res;
    }
    press(key) {
    ...
    #keyup({ data: event }) {
        ...
        switch(event.key){
            ...
            case "moveRight":
                ...
                break;
            case "softDrop":
                this.state |= _constants.constants.flags.ACTION_SOFTDROP;
    ...
    #__internal_shift() {
        if (!this.#isSleep()) {
            ...
            if ((0, _kicks.legal)(this.falling.absoluteAt({
                ...
                if (this.falling.lockResets < 31) this.falling.lockResets++;
                this.#clearFlags(_constants.constants.flags.ROTATION_ALL | _constants.constants.flags.STATE_WALL);
                // fallingdirty = true;
                if (this.#is20G()) this.#slamToFloor();
                if (!this.#shouldLock()) this.falling.locking = 0;
                return true;
            } else {
                this.state |= _constants.constants.flags.STATE_WALL;
                return false;
            }
        }
    }
    ...
    tick(frames) {
        ...
        Object.keys(this.dynamic).forEach((key)=>this.dynamic[key].tick());
        return {
            ...this.flushRes()
        };
    }
    ...
    getPreview(piece) {
        return _tetromino.tetrominoes[piece.toLowerCase()].preview;
    }
    connect(blocks) {
        const exists = (x, y)=>!!blocks.find(([a, b])=>a === x && y === b);
        return blocks.map(([x, y])=>{
            ...
            if (state === 0b1001 && !exists(x + 1, y + 1) || state === 0b1100 && !exists(x - 1, y + 1) || state === 0b0011 && !exists(x + 1, y - 1) || state === 0b0110 && !exists(x - 1, y - 1) || state === 0b0010 && !exists(x + 1, y - 1) && !exists(x - 1, y - 1) || state === 0b0001 && !exists(x + 1, y - 1) && !exists(x + 1, y + 1) || state === 0b1000 && !exists(x - 1, y + 1) && !exists(x + 1, y + 1) || state === 0b0100 && !exists(x - 1, y - 1) && !exists(x - 1, y + 1)) {
                state |= 0b1_0000;
            }
            return [
                x,
    ...
    getConnectedPreview(piece) {
        const data = _tetromino.tetrominoes[piece.toLowerCase()].preview;
        return {
            ...data,
    ...
    /** @deprecated Engine.onQueuePieces is deprecated and no longer functional. Switch to Engine.events.on("queue.add", (pieces) => {}) instead. */ onQueuePieces(_listener) {
        console.log(`${_chalk.default.redBright("[Triangle.js]")} Engine.onQueuePieces is deprecated and no longer functional. Switch to Engine.events.on("queue.add", (pieces) => {}) instead.`);
    }
    static colorMap = {
        i: _chalk.default.bgCyan,
        j: _chalk.default.bgBlue,
        l: _chalk.default.bgYellow,
        o: _chalk.default.bgWhite,
        s: _chalk.default.bgGreenBright,
        t: _chalk.default.bgMagentaBright,
        z: _chalk.default.bgRedBright,
        gb: _chalk.default.bgBlackBright,
        bomb: _chalk.default.bgHex("#FFA500")
    };
    get text() {
        ...
        for(let i = 0; i < height; i++){
            let str = i % 2 === 0 ? "|" : " ";
            if (i < this.garbageQueue.size) str += " " + _chalk.default.bgRed(" ") + " ";
            else str += "   ";
            if (i > boardTop) {
